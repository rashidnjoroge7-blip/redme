    ...
    <section className="min-h-screen">
      ...
      {error ? (
        ...
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-5 p-4 sm:grid-cols-2 lg:grid-cols-4">
          {posts.map((post) => (
            <article key={post.id} className="glass glass-hover group overflow-hidden rounded-3xl">
              {post.imageUrl ? (
              ...
              <div className="p-5">
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#ff2442]">{post.category}</p>
                <h2 className="mt-2 font-bold leading-snug text-[#1a1a1a]">{post.title}</h2>
                {post.description && (
                  <p className="mt-2 line-clamp-3 text-sm leading-relaxed text-neutral-600">{post.description}</p>
                )}
